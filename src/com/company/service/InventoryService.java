...
public class InventoryService {
    ...
    public static void equippingPlayer(Player player, Item item) {
        ...
        System.out.println("Value Item chosen =  " + player.getEquipment().get("head").getItemHabilities().get("attack"));
        System.out.println();
       ...
       // TotalPropiedades(player);


    }

   /* public static void TotalPropiedades(Player player) {
        System.out.println("Item chosen =  " + player.getEquipment().get("head").getItemHabilities().get("attack"));

        for (Map.Entry<String, Item> entry : player.getEquipment().entrySet()) {
            String key = entry.getKey();
            System.out.println("key" + "=" + key + " ");

            HashMap<String, Integer> value = entry.getValue().getItemHabilities();
            int totalStrength =0 ;
            for( Integer valor: value.values()){
                System.out.println("value " + "=" + value.values() + " ");
                System.out.println("valor " +valor);

                 totalStrength +=  valor;

            }
            System.out.println("STRENGTH = " + (totalStrength + player.getStrength()));
        }
